...
import type {
  FoundryCharacterSheetOpenedEvent
...
export class SystemEventAdapter implements InitialisableService {
  ...
  /**
   * Register application-related hooks (including character sheets)
   */
  private registerApplicationHooks(): void {
    // Character sheet rendering
    this.registerHook('renderActorSheetV2', this.handleActorSheetRender.bind(this));
    this.registerHook('closeActorSheet', this.handleActorSheetClose.bind(this));

    this.logger.debug('Application hooks registered');
  }
  ...
  private handleActorSheetRender(app: ActorSheet, html: HTMLElement, context: any, options: any): void {
    this.logger.debug(`Character sheet opened for actor`, {
      app: app,
      html: html,
      context: context,
      options: options,
    ...
    if (!app?.actor?.id) {
      this.logger.debug('No actor found in sheet render event');

      return;
    }

    const event: FoundryCharacterSheetOpenedEvent = {
      eventId: foundry.utils.randomID(),
      actorId: app.actor.id,
      actorName: app.actor.name ?? 'Eldritch',
      tokenId: options?.token?.id ?? null, 
      appId: app.id
    };

    this.eventBus.emit('characterSheet:render', event);
  }

  private handleActorSheetClose(app: ActorSheet, html: HTMLElement): void {
