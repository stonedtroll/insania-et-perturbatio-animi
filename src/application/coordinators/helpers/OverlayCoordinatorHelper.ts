...
import { MODULE_ID, ANGUISH_ARC_CONFIG } from '../../../config.js';
    ...
    private buildContextOptions(
        ...
        switch (overlayId) {
            case 'anguish-arc':
                ...
                const anguish = targetToken.actor.getAnguish();
                const config = ANGUISH_ARC_CONFIG;

                const anguishArcStartAngle = config.geometry.startAngle;
                const anguishArcEndAngle = anguishArcStartAngle + 
                    ((anguish.percentage / 100) * config.geometry.totalAngleRange);

                const anguishPercentage = anguish.percentage;

                const backgroundStartAngle = anguishArcEndAngle;
                const backgroundEndAngle = config.geometry.startAngle + config.geometry.totalAngleRange;

                return {
                    anguishArcStartAngle,
                    anguishArcEndAngle,
                    anguishPercentage,
                    lowAnguishArcColour: config.colours.low,
                    midAnguishArcColour: config.colours.mid,
                    highAnguishArcColour: config.colours.high,
                    backgroundStartAngle,
                    backgroundEndAngle,
                    backgroundColour: config.colours.background,
                    arcRadius: targetToken.radius + config.geometry.radiusOffset,
                    arcWidth: config.geometry.arcWidth,
                    anticlockwise: false
                };
